...
import { actorDetailQuery, actorCreditsQuery } from "@/entities/actor";
...
export function ActorDetailsPage() {
  ...
  const { data: actor, isLoading: isActorLoading, isError: isActorError } = useQuery(actorDetailQuery(id));
  const { data: credits, isLoading: isCreditsLoading } = useQuery(actorCreditsQuery(id));
  ...
  useEffect(() => {
    if (!isNaN(id)) {
      ...
      if (stored) {
        try {
          const list: ActorDetails[] = JSON.parse(stored);
  ...
  if (isActorError || !actor) {
    return (
      <div className="text-center py-12">
        ...
        <button
          onClick={() => navigate(-1)}
          className="mt-4 text-purple-600 hover:underline inline-flex items-center gap-2 cursor-pointer"
        >
  ...
  return (
    <div className="space-y-8">
      <button
        onClick={() => navigate(-1)}
        className="inline-flex items-center gap-2 text-purple-600 hover:text-purple-700 font-semibold transition cursor-pointer"
      >
        <ArrowLeft className="w-4 h-4" /> Назад
      </button>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold transition cursor-pointer ${
                isFavorite
